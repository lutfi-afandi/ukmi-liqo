        ...
        <div class="col-md-12">
            <div class="info-box p-2 pl-3">
                ...
                <div class="info-box-button pt-2 pr-3">
                    <a href="{{ route('admin.dokumen.index') }}" class="btn btn-info"><i
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive mt-2">
                        <table class="table table-striped table-bordered" id="datatableku" width="100%">
                            ...
                            <tbody>
                                @if ($sub_dokumens->isEmpty())
                                    <tr>
                                        <td colspan="4" class="text-center">Belum ada data!</td>

                                    </tr>
                                @else
                                    @foreach ($sub_dokumens as $item)
                                        <tr>
                                            <td class="text-center">{{ $loop->iteration }}</td>
                                            <td>{{ $item->nama_subdok }}</td>
                                            <td>
                                                <button onclick="lihat_berkas('{{ $item->file_subdok }}')"
                                                    class="btn btn-sm btn-warning">
                                                    {{ $item->nama_subdok }} <i class="fas fa-share-square"></i></button>
                                            </td>
    ...
    <div class="modal fade" id="modal-default">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.sub_dokumen.store') }}" method="post">
                    @csrf
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="nama">Nama Dokumen</label>
                            <input type="hidden" name="dokumen_id" value="{{ $dokumen->id }}">
                            <input type="text" class="form-control" name="nama_subdok" id="nama_subdok">
                        </div>
                        <div class="form-group">
                            <label for="nama">Link Dokumen</label>
                            <input type="text" class="form-control" name="file_subdok" id="file_subdok">
                        </div>
                    </div>
...
@section('js')
    <script>
        ...
        function ubah(id) {
            var url = "{{ route('admin.sub_dokumen.edit', ':id') }}";
        ...
        function hapus(id) {
            var url = "{{ route('admin.sub_dokumen.show', ':id') }}";
            url = url.replace(":id", id);
            $.ajax({
                type: "get",
                url: url,
