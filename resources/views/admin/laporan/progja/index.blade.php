        ...
        <div class="col-md-12">
            <div class="info-box p-4">
                ...
                <div class="info-box-content">
                    ...
                    <h5 class="info-box-text font-weight-light">{{ $progja->user->name }}</h4>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control form-control-border"
                            value="{{ date('d/m/Y H:i', strtotime($progja->tgl_upload_progja)) }}" readonly>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>Status : </label>
                        <input type="text" class="form-control form-control-border" value="{{ $progja->konf_progja }}"
                            readonly>
                    </div>
                    <form action="{{ route('admin.progja.update', ['progja' => $progja->id]) }}" method="post">
                        @csrf
                        @method('put')
                        <div class="form-group">
                            <label>Set Status:</label>
                            <select class="form-control" name="konf_progja" required>
                                <option value="" hidden>-Pilih Status-</option>
                                <option value="sedang" {{ $progja->konf_progja == 'sedang' ? 'selected' : '' }}>diperiksa
                                </option>
                                <option {{ $progja->konf_progja == 'diterima' ? 'selected' : '' }}>diterima</option>
                                <option {{ $progja->konf_progja == 'ditolak' ? 'selected' : '' }}>ditolak</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Catatan Aditor:</label>
                            <textarea id="summernote" name="ket_progja" placeholder="catatan Auditor">
                                {!! $progja->ket_progja ?? '' !!}
                            </textarea>
                        </div>
                        <div class="form-group">
