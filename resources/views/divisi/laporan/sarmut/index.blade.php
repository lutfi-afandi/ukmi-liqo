...
@php
    ...
    if ($laporan == null) {
        ...
        $rute_update = route('divisi.sarmut.update_sarmut', ['sarmut' => $laporan->id]);
        $rute_reupload = route('divisi.sarmut.reupload', ['sarmut' => $laporan->id]);
    }
        ...
        <div class="col-lg-10 ">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title text-lg font-weight-bold">Sasaran Mutu Tahun : {{ $periode->tahun }}</h3>
                </div>
                <div class="card-body">
                    <div id="alert">
                        @if (session()->has('msgs'))
                    ...
                    @if ($laporan->sarmut == null)
                        <div class="callout callout-danger">
                            <h5>Belum ada laporan</h5>
                            <p class="font-italic">{{ $laporan->name }} belum mengunggah dokumen Sasaran mutu Periode
                                {{ $periode->tahun }}!</p>
                            <form action="{{ route('divisi.sarmut.update', ['sarmut' => $laporan->id]) }}" method="post"
                    ...
                    @else
                        <dl class="row text-md">
                            ...
                            <dd class="col-sm-8">:
                                <a type="button" class="text-{!! $helper->bg($laporan->konf_sarmut) !!}">
                                    {!! $helper->icon($laporan->konf_sarmut) !!}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8">:
                                <a type="button">
                                    {{ date('d/m/Y', strtotime($laporan->tgl_upload_sarmut)) }}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8">:
                                <a type="button">
                                    {{ $laporan->konf_sarmut == 'diterima' ? date('d/m/Y', strtotime($laporan->tgl_konf_sarmut)) : '' }}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8 d-inline-flex">:
                                <div class="callout callout-danger w-100">
                                    {!! $laporan->ket_sarmut ?? '<i>belum ada catatan</i>' !!}
                                </div>
                            </dd>

                            <dt class="col-sm-3">Aksi</dt>
                            <dd class="col-sm-8">:
                                @if ($laporan->konf_sarmut == 'diterima')
                                    <i class="fas fa-check-circle"></i> selesai
                                @elseif($laporan->konf_sarmut == 'sedang')
                                    <i class="fas fa-info-circle fa-beat text-info"></i> Menunggu pemeriksaan dari LPM
                                @elseif($laporan->konf_sarmut == 'belum')
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-gradient-primary">
                    <h4 class="modal-title">Update Sasaran Mutu - {{ $user->name }}</h4>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-gradient-navy">
                    <h4 class="modal-title">Reupload Sasaran Mutu - {{ $user->name }}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('js')
    <script>
        ...
        function riwayat(param_id) {
            var url = "{{ route('divisi.sarmut.riwayat', ':id') }}";
