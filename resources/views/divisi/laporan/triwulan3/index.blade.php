...
@php
    ...
    if ($laporan == null) {
        ...
        $rute_update = route('divisi.triwulan3.update', ['triwulan3' => $laporan->id]);
        $rute_reupload = route('divisi.triwulan3.reupload', ['triwulan3' => $laporan->id]);
    }
@endphp
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="info-box p-4">
                ...
                <div class="info-box-content">
                    <h4 class="info-box-number mb-0">Detail Laporan Triwulan 3</h4>
        ...
        <div class="col-lg-10">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title text-lg font-weight-bold">Laporan Triwulan 3 : {{ $periode->tahun }}</h3>
                </div>
                <div class="card-body">
                    <div id="alert">
                        @if (session()->has('msgs'))
                    ...
                    @if ($triwulan3 == null)
                        <div class="callout callout-danger">
                            ...
                            <p class="font-italic">{{ $laporan->user->name }} belum mengunggah dokumen Laporan Triwulan 3
                                Periode
                                {{ $periode->tahun }}!</p>
                            <form action="{{ route('divisi.triwulan3.store') }}" method="post"
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        ...
                                        <input type="file" class="form-control" id="file_tw3" name="file_tw3"
                    ...
                    @else
                        <dl class="row text-md">
                            ...
                            <dd class="col-sm-8">:
                                <a type="button" class="" onclick="lihat('{{ $triwulan3->file_tw3 }}')">
                                    {{ $triwulan3->file_tw3 }} <i class="fas fa-share-square"></i>
                                </a>
                            </dd>
                            <dt class="col-sm-3">Status</dt>
                            <dd class="col-sm-8">:
                                <a type="button" class="text-{!! $helper->bg($triwulan3->konf) !!}">
                                    {!! $helper->icon($triwulan3->konf) !!}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8">:
                                <a type="button">
                                    {{ date('d/m/Y', strtotime($triwulan3->tgl_upload)) }}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8">:
                                <a type="button">
                                    {{ $triwulan3->konf == 'diterima' ? date('d/m/Y', strtotime($triwulan3->tgl_konf)) : '' }}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8 d-inline-flex">:
                                <div class="callout callout-danger w-100">
                                    {!! $triwulan3->ket ?? '<i>belum ada catatan</i>' !!}
                                </div>
                            </dd>

                            <dt class="col-sm-3">Aksi</dt>
                            <dd class="col-sm-8">:
                                @if ($triwulan3->konf == 'diterima')
                                    <i class="fas fa-check-circle"></i> selesai
                                @elseif($triwulan3->konf == 'sedang')
                                    <i class="fas fa-info-circle fa-beat text-info"></i> Menunggu pemeriksaan dari LPM
                                @elseif($triwulan3->konf == 'belum')
                                    <button type="button" class="btn btn-info btn-sm"
                                        onclick="ubah('{{ $triwulan3->id }}')">
                                        <i class="far fa-edit"></i> ubah
                                    </button>
                                @else
                                    <button type="button" class="btn btn-info btn-sm"
                                        onclick="reupload('{{ $triwulan3->id }}')">
                                        <i class="fas fa-retweet"></i> reupload
                                    </button>
                                @endif
                                <button type="button" class="btn bg-navy btn-sm"
                                    onclick="riwayat('{{ $triwulan3->id }}')">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-gradient-primary">
                    <h4 class="modal-title">Update Laporan Triwulan 3 - {{ $user->name }}</h4>
                ...
                <form action="{{ $rute_update }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="file_tw3">File Triwulan 3</label>
                            <input type="file" class="form-control" id="file_tw3" name="file_tw3"
                                accept="application/pdf">
                        </div>
                    </div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-gradient-navy">
                    <h4 class="modal-title">Reupload Laporan Triwulan 3 - {{ $user->name }}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('js')
    <script>
        function lihat(parameter_file) {
            ...
            var storage = "{{ asset('storage/uploads/file_tw3/') }}/";
        ...
        function riwayat(param_id) {
            var url = "{{ route('divisi.triwulan3.riwayat', ':id') }}";
