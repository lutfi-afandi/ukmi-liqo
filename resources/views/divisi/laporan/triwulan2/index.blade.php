        ...
        <div class="col-lg-10">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title text-lg font-weight-bold">Laporan Triwulan 2 : {{ $periode->tahun }}</h3>
                </div>
                <div class="card-body">
                    <div id="alert">
                        @if (session()->has('msgs'))
                    ...
                    @if ($laporan->triwulan2 == null)
                        <div class="callout callout-danger">
                            <h5>Belum ada laporan</h5>
                            <p class="font-italic">{{ $laporan->user->name }} belum mengunggah dokumen Laporan Triwulan 2
                                Periode
                                {{ $periode->tahun }}!</p>
                            <form action="{{ route('divisi.triwulan2.store') }}" method="post"
                    ...
                    @else
                        <dl class="row text-md">
                            ...
                            <dd class="col-sm-8">:
                                <a type="button" class="text-{!! $helper->bg($triwulan2->konf) !!}">
                                    {!! $helper->icon($triwulan2->konf) !!}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8">:
                                <a type="button">
                                    {{ date('d/m/Y', strtotime($triwulan2->tgl_upload)) }}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8">:
                                <a type="button">
                                    {{ $triwulan2->konf == 'diterima' ? date('d/m/Y', strtotime($triwulan2->tgl_konf)) : '' }}
                                </a>
                            </dd>
                            ...
                            <dd class="col-sm-8 d-inline-flex">:
                                <div class="callout callout-danger w-100">
                                    {!! $triwulan2->ket ?? '<i>belum ada catatan</i>' !!}
                                </div>
                            </dd>

                            <dt class="col-sm-3">Aksi</dt>
                            <dd class="col-sm-8">:
                                @if ($triwulan2->konf == 'diterima')
                                    <i class="fas fa-check-circle"></i> selesai
                                @elseif($triwulan2->konf == 'sedang')
                                    <i class="fas fa-info-circle fa-beat text-info"></i> Menunggu pemeriksaan dari LPM
                                @elseif($triwulan2->konf == 'belum')
                                    <button type="button" class="btn btn-info btn-sm"
                                        onclick="ubah('{{ $triwulan2->id }}')">
                                        <i class="far fa-edit"></i> ubah
                                    </button>
                                @else
                                    <button type="button" class="btn btn-info btn-sm"
                                        onclick="reupload('{{ $triwulan2->id }}')">
                                        <i class="fas fa-retweet"></i> reupload
                                    </button>
                                @endif
                                <button type="button" class="btn bg-navy btn-sm"
                                    onclick="riwayat('{{ $triwulan2->id }}')">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-gradient-primary">
                    <h4 class="modal-title">Update Laporan Triwulan 2 - {{ $user->name }}</h4>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-gradient-navy">
                    <h4 class="modal-title">Reupload Laporan Triwulan 2 - {{ $user->name }}</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
@section('js')
    <script>
        ...
        function riwayat(param_id) {
            var url = "{{ route('divisi.triwulan2.riwayat', ':id') }}";
